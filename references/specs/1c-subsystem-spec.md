...
- Сам формат не задаёт предела глубины (на практике встречается 3–4 уровня).
  Unica доказывает и публикует не более восьми программных имён в одном
  `SubsystemAddress`; более глубокая зарегистрированная цепочка считается
  недоступной целиком, а не обрезается.
...
Для активного профиля `2.20` `IncludeInCommandInterface` обязан присутствовать ровно один раз
в `Properties` каждой зарегистрированной подсистемы и содержать ровно `true` или `false`.
Отсутствие элемента, дубликат и другое написание логического значения не задают
значение по умолчанию: доказательство зарегистрированной топологии в этом случае
...
Например, вложенная подсистема в платформенном XML записывается как
`Subsystem.СтандартныеПодсистемы.Subsystem.Обсуждения`. Это платформенный формат
ссылки; плоский публичный адрес Unica описывается контрактом инструмента, а не
подменяет XML-значение.
