...
> Права `Interactive*PredefinedData` (интерактивное удаление и пометка удаления предопределённых) позволяют пользователю менять предопределённые данные, которые являются частью конфигурации. Обычно выдавать их не следует; `role-validate` предупреждает о каждом таком праве со значением `true`.
