...
Платформа 8.3.27 способна импортировать некоторые прежние выгрузки, но это не меняет текущий контракт Unica: все version-bearing XML генерируются как `2.20`, а старые исходники доступны только для чтения до явной перевыгрузки пользователем через 1С 8.3.27.
